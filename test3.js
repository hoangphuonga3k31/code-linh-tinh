if (eventData.newValue.value > 0 && eventData.oldValue.value > 0 && eventData.newValue.value > eventData.oldValue.value) {
    ...
    if (production_master.getRowCount() > 0) {
        ...
        if (production_master.rows[0].PART_NUMBER_OF_BILLET_STATUS != 'D') {
        ...
        let isNewLot = false;
        if (me.X1_4M1_A3C05 == 1) {
            isNewLot = true;
        }
        // if (production_master.rows[0].MATERIAL_LOT_NUMBER.replace(/\s/g, "") != BilletInfoData.rows[0].MATERIAL_LOT_NUMBER.replace(/\s/g, "")) {
        //     isNewLot = true;
        // }
        ...
        production_master.rows[0].NUMBER_BILLET_TREE_CUT = me.X1_4M1_A3C05;
        ...
        if (isNewLot) {
            let PBM_Data = me.LoadingData({
                strQuery: "select * from PRODUCTION_BILLET_MASTER where MATERIAL_LOT_NUMBER = '" + BilletInfoData.rows[0].MATERIAL_LOT_NUMBER + "'"
            });
            PBM_Data.rows[0].STATUS = "C";
            PBM_Data.rows[0].UPDATED_DATE = new Date();
            PBM_Data.rows[0].UPDATED_BY = Resources["CurrentSessionInfo"].GetCurrentUser();

            Things["CTA.Business.Categories.PRODUCTION_BILLET_MASTER"].DataTable_Execute = PBM_Data;
            Things["CTA.Business.Categories.PRODUCTION_BILLET_MASTER"].SetFlagExecute({
                stateFlag: "EDIT" /* STRING [Required] */,
            });
            Things["CTA.Business.Categories.PRODUCTION_BILLET_MASTER"].ExecuteData()
        }
