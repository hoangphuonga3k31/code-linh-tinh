try {
	...
	let currentDatetimeToString =
		currentDatetime.getFullYear().toString() +
		"-" +
		(parseInt(currentDatetime.getMonth()) + 1).toString().padStart(2, "0") +
		"-" +
	...
	let yesterdayDatetimeToString =
		startTime.getFullYear().toString() +
		"-" +
		(parseInt(startTime.getMonth()) + 1).toString().padStart(2, "0") +
		"-" +
			...
			importedPODetailData.forEach((row) => {
				if (Object.keys(row).length !== 0) {
					...
					if (!parseFloat(PRODUCT_SIZE_DataArray[2])) {
						throw (
							"Dữ liệu Kích thước sản phẩm trong Code sản phẩm " +
							row.PO_DETAILS_PROD_CODE.replace(/\s/g, "") +
							" sai định dạng (" +
							PRODUCT_SIZE_DataArray[2] +
							")"
						);
					} else {
						...
						if (PRODUCT_SIZE < PRODUCT_SIZE_MIN) {
							throw (
								"Dữ liệu Kích thước sản phẩm trong Code sản phẩm " +
								row.PO_DETAILS_PROD_CODE.replace(/\s/g, "") +
								" có Kích thước nhỏ hơn quy định (" +
								PRODUCT_SIZE_MIN / 1000 +
								"m)"
							);
						}
						if (PRODUCT_SIZE > PRODUCT_SIZE_MAX) {
							throw (
								"Dữ liệu Kích thước sản phẩm trong Code sản phẩm " +
								row.PO_DETAILS_PROD_CODE.replace(/\s/g, "") +
								" có Kích thước lớn hơn quy định (" +
					...
					let checkPODetailDataExist = me.LoadingData({
						strQuery:
							"select * from PURCHASE_ORDER_DETAILS where fr_key = " +
							POIDData +
							" and PO_DETAILS_PROD_CODE = '" +
							row.PO_DETAILS_PROD_CODE +
							"'" /* STRING [Required] */,
					});
						...
						if (checkWO.getRowCount() > 0) {
							checkWO.rows.forEach((WO_row) => {
								let checkProductionMasterDetail = me.LoadingData({
									strQuery:
										"select * from PRODUCTION_MASTER_DETAIL where WORK_ORDER_ID = " +
										WO_row.PR_KEY +
										" and PRODUCT_LOT_NUMBER = '" +
										row.LOT_NUMBER +
										"' and PALLET_ID = '" +
										row.PALLET_ID +
										"'",
		...
		for (let i = 0; i < DSHoaGiaJsonData.length; i++) {
			if (Object.keys(DSHoaGiaJsonData[i]).toString().length !== 0) {
				let checkLotNumberData = me.LoadingData({
					strQuery:
						"select * from WHM_PRODUCT_MASTER where LOT_NUMBER = '" +
						DSHoaGiaJsonData[i].LOT_NUMBER.toString().trim() +
						"' and PALLET_ID = '" +
						DSHoaGiaJsonData[i].PALLET_ID.toString().trim() +
						"'",
				});
				...
				let checkPOExist = me.LoadingData({
					strQuery:
						"select PR_KEY from PURCHASE_ORDER where PURCHASE_ORDER_CODE = '" + DSHoaGiaJsonData[i].PURCHASE_ORDER_CODE + "'" /* STRING [Required] */,
				});
				if (checkPOExist.getRowCount() > 0) {
					let checkPODetailDataExist = me.LoadingData({
						strQuery:
							"select * from PURCHASE_ORDER_DETAILS where fr_key = " +
							checkPOExist.rows[0].PR_KEY +
							" and PO_DETAILS_PROD_CODE = '" +
							DSHoaGiaJsonData[i].PO_DETAILS_PROD_CODE +
							"'" /* STRING [Required] */,
					});
					if (checkPODetailDataExist.getRowCount() > 0) {
						UpdatingPODetailData.rows.forEach((UpdatingPODetailDataRow) => {
							if (UpdatingPODetailDataRow.PO_DETAILS_PROD_CODE.toString().trim() == DSHoaGiaJsonData[i].PO_DETAILS_PROD_CODE.toString().trim()) {
								Things["CTA.Business.Product.PO.PurchaseOrder"].AllImportedDataForAging.rows.forEach((POAllImportedDataRow) => {
									if (
										UpdatingPODetailDataRow.FR_KEY == POAllImportedDataRow.PR_KEY &&
										DSHoaGiaJsonData[i].PURCHASE_ORDER_CODE.toString().trim() == POAllImportedDataRow.PURCHASE_ORDER_CODE.toString().trim()
									) {
						...
						NewPODetailData.rows.forEach((NewImportedDataRow) => {
							if (NewImportedDataRow.PO_DETAILS_PROD_CODE.toString().trim() == DSHoaGiaJsonData[i].PO_DETAILS_PROD_CODE.toString().trim()) {
								Things["CTA.Business.Product.PO.PurchaseOrder"].AllImportedDataForAging.rows.forEach((POAllImportedDataRow) => {
									if (
										NewImportedDataRow.FR_KEY == POAllImportedDataRow.PR_KEY &&
										DSHoaGiaJsonData[i].PURCHASE_ORDER_CODE.toString().trim() == POAllImportedDataRow.PURCHASE_ORDER_CODE.toString().trim()
									) {
				...
				if (DataForImporting_WHM_PRODUCT_MASTER.getRowCount() > 0) {
					...
					DataForImporting_WHM_PRODUCT_MASTER.rows.forEach((DataRow) => {
						if (DataRow.PALLET_ID == DSHoaGiaJsonData[i].PALLET_ID && DSHoaGiaJsonData[i].LOT_NUMBER == DataRow.LOT_NUMBER) {
							if (
								DataRow.QUANTITY_INCOME == DSHoaGiaJsonData[i].QUANTITY_INCOME &&
